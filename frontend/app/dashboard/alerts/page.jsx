...
export default function AlertsPage() {
    ...
    const handleCreateAlert = async (e) => { e.preventDefault(); if (!title || keywords.length === 0 || platforms.length === 0) { setFormError('Title, keyword, and platform are required.'); return; } try { const newAlert = { title, description, severity, keywords, platforms }; await api('alerts', { method: 'POST', body: JSON.stringify(newAlert) }); setTitle(''); setDescription(''); setSeverity('Medium'); setKeywords([]); setPlatforms([]); setFormError(''); await fetchAlerts(); } catch { setFormError('Failed to create alert.'); } };
    ...
    const handleUpdateAlert = async (id, updatedData) => { try { await api(`alerts/${id}`, { method: 'PUT', body: JSON.stringify(updatedData) }); await fetchAlerts(); handleCloseModal(); } catch (error) { console.error('Failed to update alert:', error); } };

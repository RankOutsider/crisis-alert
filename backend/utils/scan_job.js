...
const runScanJob = async () => {
    ...
    console.log("");
    console.log("==========================================================");
    console.log(`[${new Date().toISOString()}] 🤖 Starting periodically scanning job...`);
    ...
    try {
        ...
        if (activeAlerts.length === 0) {
            console.log("➡️ No ACTIVE Alerts. Closing Scanning Job.");
        ...
        // Set này CHỈ chứa các link đã có trong DB
        const dbLinks = new Set(
            existingLinksRaw.map(link => `${link.AlertId}-${link.PostId}`)
        );
        console.log(`🔎 Found ${activeAlerts.length} ACTIVE Alerts. Loaded ${dbLinks.size} existing associations from DB.`);
        ...
        if (allPostsToScan.length === 0) {
            console.log("➡️ No new posts found to scan. Closing Scanning Job.");
            await sequelize.close(); return;
        }
        console.log(`🔎 Found ${allPostsToScan.length} Posts (since ${startOfEarliestMonth.toISOString().split('T')[0]}) to scan.`);
        ...
        for (const alert of activeAlerts) {
            ...
            for (const post of allPostsToScan) {
                ...
                // Kiểm tra liên kết trong Set (lấy từ DB)
                const linkKey = `${alert.id}-${post.id}`;
                if (dbLinks.has(linkKey)) continue; // Bỏ qua nếu đã có trong DB

                // Nếu chưa có, kiểm tra keyword và platform
                ...
                if (keywordMatch && platformMatch) {
                    ...
                    // (FIX) Thêm vào dbLinks (in-memory) NGAY LẬP TỨC
                    // để Alert tiếp theo (nếu có) không bị trùng
                    dbLinks.add(linkKey);
                }
            }
            ...
            if (newPostsToLink.length > 0) {
                // Gói việc lưu DB và gửi Mail vào Try/Catch
                // để nếu Mail lỗi, DB vẫn được lưu
                try {
                    ...
                    continue; // Bỏ qua Alert này nếu không lưu được
                }

                // Gửi email (vẫn có delay)
        ...
        console.log(`[${new Date().toISOString()}] 🤖 Closed DB connection.`);
    }
};
